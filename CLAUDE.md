...
- Concentrated bid-side "price support" liquidity at graduation is drainable — do NOT add it. A tight ETH wall just under launch price (to "cushion sells") is a pot of ETH at high prices; an attacker acquires tokens cheaply and dumps them into the wall, extracting the raise at near-P fills. Based Alpha (based-hood) shipped exactly this ([0.5P,P] ETH wall) and got drained across 23 tokens in 2 blocks on 2026-07-19 (<5 ETH; they retrofitted a deep ~0.05P→P spread). LittleJohn is immune because we seed one **constant-product** pool at the graduation price and burn the LP — no concentrated pot to drain. If "price support" is ever proposed, it must be constant-product or a deep-spread ladder, NEVER a concentrated wall. (Related: our `LaunchToken` transfer-locks curve tokens until graduation, so no rogue side-pool can be pre-seeded — keep that.)
